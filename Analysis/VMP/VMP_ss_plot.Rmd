...
df = df %>% filter(!subject %in% c("16") & subject < 50)
library(dplyr)
library(cmdstanr)
library(here)
library(furrr)
...
# Precompile model outside the loop — avoids recompiling for each subject
# mod <- cmdstanr::cmdstan_model(here::here("Stanmodels", "ss", "ACCSS_Correct.stan"))
mod <- cmdstanr::cmdstan_model(here::here("Stanmodels", "ss", "BINSS_Correct.stan"))
...
df1 = df %>% filter(!subject %in% c("16") & subject < 5)
...
source(here::here("Analysis","Functions","Plot_ACC_Hier.R"))

preds = Get_predictive_ACC_hier(cor,df1)

Plot_psychometric_ACC_hier(preds,df1, bin = 10)

Plot_RT_ACC_hier(preds,df1, bin = 10)

Plot_Conf_ACC_hier(preds,df1, bin = 10)

```

```{r, fig.height=7,fig.width=12, warning=FALSE, message=FALSE}
group_pred = Get_predictive_group(cor,df1,n_draws = 2000)

plots = Plot_group_predictive(group_pred, df1)
...
Get_predictive = function(fit, df, n_draws, model = "hierarchical"){
  ...
  pred_list <- future_lapply(subjects, function(s) {
    lapply(draws, function(d) {
      ...
      psycho_ACC = function(x, alpha, beta, lapse){
        lapse + (1 - 2 * lapse) * plogis(beta * (x - alpha))
      }
