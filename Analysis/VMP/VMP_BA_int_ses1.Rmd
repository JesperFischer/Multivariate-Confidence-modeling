...
removers_vmp2 = c("249","258","338","344","336","387","426","409","399")
```
...
# df1 = df %>% filter(!subject %in% removers_vmp2 & cohort == "vmp2")
df1 = df %>% filter(!subject %in% c("249","258","344","399") & cohort == "vmp2") %>% 
...
cor$save_object(here::here("Saved models","VMP_BA_int_ses1_chrt2.rds"))
```
...
mod = cmdstanr::cmdstan_model(here::here("Stanmodels","ss","Pure","psycho_rt.stan"))
...
rt = psyrt$loo("log_lik_bin")
psycho = psy$loo("log_lik_bin")
full = cor$loo("log_lik_bin")

loo::loo_compare(list(psy = psycho, full = full, rt = rt))

full = cor$loo("log_lik_rt")
rt = psyrt$loo("log_lik_rt")

loo::loo_compare(list(full = full, rt = rt))

full = cor$loo("log_lik")
...
library(ggh4x)
rbind(
  get_psychometric(psy, "normal"),
  get_psychometric(psyrt, "rt"),
  get_psychometric(cor, "confidence")
) %>% 
  ggplot(aes(y = subject, x = value, fill = model))+
    geom_density_ridges(aes(y = subject, x = value),rel_min_height = 0.01, alpha = 0.5)+
    facet_wrap(~variable, scales = "free")+
    facetted_pos_scales(
    x = list(
      variable == "alpha"  ~ scale_x_continuous(limits = c(-40, 20), breaks = seq(-40, 10, 10)),
      variable == "beta"   ~ scale_x_continuous(limits = c(-3, 1), breaks = seq(-3, 1, 1)),
      variable == "lapse"  ~ scale_x_continuous(limits = c(0, 0.15), breaks = seq(0, 0.15, 0.05))
    )
  ) +
  theme_classic(base_size = 16)


get_copula = function(fit){
  
  df = as_draws_df(fit$draws(c("rho_p_rt","rho_p_conf","rho_rt_conf"))) %>% 
    select(-contains(".")) %>%
    mutate(draw = 1:n()) %>%
    pivot_longer(-draw) %>%
    extract(name, into = c("variable", "subject"),
  ...
  group = df %>% group_by(variable,draw) %>% summarize(value = mean(value)) %>% mutate(subject = 0)
...
df[[1]] %>% 
  ggplot(aes(y = subject, x = value))+
    geom_density_ridges(aes(y = subject, x = value),rel_min_height = 0.01, alpha = 0.5)+
    geom_density_ridges(data = df[[2]], aes(y = subject, x = value),rel_min_height = 0.01, alpha = 0.5, scale = 0.2, fill = "red")+
    facet_wrap(~variable, scales = "free")+
    facetted_pos_scales(
    x = list(
      variable == "alpha"  ~ scale_x_continuous(limits = c(-40, 20), breaks = seq(-40, 10, 10)),
...
plot_subjects(df1,18, bin = 7)
df1 %>% filter(subject == 18) %>% ggplot(aes(x = Correct, y = Confidence))+geom_point()
df1 %>% filter(subject == 22) %>% ggplot(aes(x = Correct, y = Confidence))+geom_point()
df1 %>% filter(subject == 16) %>% ggplot(aes(x = RT, y = Confidence))+geom_point()
df1 %>% filter(subject == 25) %>% ggplot(aes(x = RT, y = Confidence))+geom_point()+geom_smooth(method = "lm")
cor.test(df1 %>% filter(subject == 25) %>% .$RT, df1 %>% filter(subject == 25) %>% .$Confidence)
```
...
```{r, fig.height=7,fig.width=12}
df1$X_scaled = (df1$X - mean(df1$X)) / sd(df1$X)

preds = Get_predictive_ACC_hier(cor,df1,20)
