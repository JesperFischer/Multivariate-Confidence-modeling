...
pure$save_object(here::here("Saved models","VMP_full_pure_vmp1.rds"))


pureloo = pure$loo()
vmp1loo = vmp1$loo("log_lik_bin")

data.frame(loo::loo_compare(list(Bin = pureloo,CBM = vmp1loo))) %>% 
...
ggsave(here::here("VMP1_Bin_CBM_full_compar.tiff"),dpi = 300,width = 11, height = 6, units = "cm")
...
```{r, fig.height=7,fig.width=12}
preds = Get_predictive_ACC_hier(fit,df1,10)

# bin_pure = Plot_psychometric_ACC_hier(preds,df1)

# bin_pure

bin = Plot_psychometric_resp_hier(preds,df1)

ggsave(here::here("all_bin.tiff"),bin ,dpi = 300,width = 34, height = 18, units = "cm")
...
ggsave(here::here("all_rt.tiff"),rt ,dpi = 300,width = 34, height = 18, units = "cm")
...
ggsave(here::here("all_conf.tiff"),conf ,dpi = 300,width = 34, height = 18, units = "cm")
...
ggsave(here::here("Extero_VMP1_marginal_means.tiff"), plots$plot_combined, dpi = 100, width = 24, height = 16)
...
pure$save_object(here::here("Saved models","VMP_full_pure_vmp2.rds"))


pureloo = pure$loo()
vmp2loo = vmp2$loo("log_lik_bin")

data.frame(loo::loo_compare(list(Bin = pureloo,CBM = vmp2loo))) %>% 
  rownames_to_column("Model") %>% 
  ggplot(aes(x = elpd_diff, xmin = elpd_diff-2*se_diff, xmax = elpd_diff + 2*se_diff, y = Model, col = Model))+
  geom_pointrange()+
  geom_vline(xintercept = 0, linetype = 2, alpha = 0.5)+
...
ggsave(here::here("VMP1_Bin_CBM_full_compar_vmp2.tiff"),dpi = 300,width = 11, height = 6, units = "cm")
...
bin = Plot_psychometric_resp_hier(preds,df1, bin = 5)
...
ggsave(here::here("Extero_VMP2_marginal_means.tiff"), plots$plot_combined, dpi = 300, width = 24, height = 16, units = "cm")
# ggsave(here::here("Extero_VMP1_marginal_means.tiff"), plots$plot_combined, dpi = 300, width = 24, height = 16, units = "cm")
