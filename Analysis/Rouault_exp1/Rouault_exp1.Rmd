...
removers = c("23","88","120","141","212","221","230","284","292","338","343","346","347","363","445","456","484")
```
...
```{r, fig.height=7,fig.width=12, message=F, error=F,warning=F}
lapply(list(1:8), function(chunk) plot_subjects(df %>% filter(!subject %in% removers), chunk, bin = 7))
lapply(list(9:20), function(chunk) plot_subjects(df %>% filter(!subject %in% removers), chunk, bin = 7))
...
```{r}
dd = Get_predictive_discrete(cor,df1)
Plot_psychometric_discrete(dd,df1,response_prob = T)
Plot_RT_quad(dd,df1)
Plot_Conf_discrete(dd,df1)
```
