...
df = df %>% filter(session == 1) %>% 
  ...
  select(ResponseCorrect,participant_id,DecisionRT,Decision, Confidence, ConfidenceRT,Modality, Alpha,cohort)  %>% drop_na()
...
df1 %>% mutate(DecisionRT = log(DecisionRT), Confidence = Confidence/100) %>% 
  filter(participant_id > 400 & participant_id < 405) %>% 
  pivot_longer(cols = c("Decision","DecisionRT","Confidence")) %>% 
  # filter(abs(Alpha)<25) %>% 
  group_by(Alpha, name,participant_id) %>% 
  summarize(mean = mean(value), se = sd(value)/sqrt(n())) %>% 
...
pps = df1 %>% group_by(participant_id,cohort) %>% summarize(n = n()) %>% filter(n > 60)
...
# 
# bind_rows(
#   df1 %>% filter(participant_id %in% pps$participant_id) %>% 
#       group_by(Alpha) %>% 
#       summarize(name = "ResponseCorrect",
#               mean = mean(Decision),
#               se = sd(Decision) / sqrt(n())),
...
removes1 = remover(df1)

df1 %>% filter(participant_id %in% removes1) %>% 
...
removes2 = remover(df1 %>% filter(participant_id %in% pps$participant_id))


df1 %>% filter(participant_id %in% pps$participant_id) %>% filter(participant_id %in% removes2) %>% 
  group_by(Alpha,participant_id) %>% summarize(pfaster = mean(Decision), se = sd(Decision)/sqrt(n())) %>% 
  ggplot(aes(x = Alpha, y = pfaster,ymin = pfaster-2*se, ymax = pfaster+2*se))+geom_pointrange()+facet_wrap(~participant_id)
...
df1 %>% filter(participant_id %in% pps$participant_id) %>% filter(participant_id %in% removes2) %>% 
  group_by(Alpha,participant_id) %>% summarize(pfaster = mean(DecisionRT), se = sd(DecisionRT)/sqrt(n())) %>% 
  ggplot(aes(x = Alpha, y = pfaster,ymin = pfaster-2*se, ymax = pfaster+2*se))+geom_pointrange()+facet_wrap(~participant_id)
...
df1 %>% filter(participant_id %in% pps$participant_id) %>% filter(participant_id %in% removes2) %>% 
  group_by(Alpha,participant_id,ResponseCorrect) %>% summarize(pfaster = mean(Confidence), se = sd(Confidence)/sqrt(n())) %>% 
  ggplot(aes(x = Alpha, y = pfaster,ymin = pfaster-2*se, ymax = pfaster+2*se,col = as.factor(ResponseCorrect)))+geom_pointrange()+facet_wrap(~participant_id)
...
removers2 = c("9","16","44","253","258","270","293","295","307","321","392","465","541")
...
qq = df1 %>% filter(participant_id %in% pps$participant_id) %>% filter(participant_id != "16")


qq = qq %>% mutate(participant_id = as.numeric(as.factor(participant_id)))
...
mod = cmdstan_model(here::here("Stanmodels","both_un.stan"))
# 
vmp2 <- mod$sample(
  data = list(N = nrow(qq),
...
vmp2$save_object(here::here("Saved models","VMP_extero_cohort1_all_meta_un_rt_un.rds"))
