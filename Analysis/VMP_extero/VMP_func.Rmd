...
df %>% 
  ...
  ggplot(aes(x = absX, y = mean, col = as.factor(Y)))+geom_point()
...
fit00 = fit_data_copula_rt(df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1),ACC = F, "VMP1_pure_correct",modeltype = "pure",conf = "ord_beta")
...
pred_med = Get_predictive(med,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1), n_draws = 10, model = "med")
...
bin_med = Plot_bin(pred_med,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1), ACC = F)

bin_med[[1]]+ggtitle("pure")
bin_med[[2]]+ggtitle("pure")
...
rt_med = Plot_RT(pred_med,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1), bin = 7)
rt_med[[1]]+ggtitle("med")
rt_med[[2]]+ggtitle("med")
rt_med[[3]]+ggtitle("med")
...
conf_med = Plot_conf(pred_med,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1), bin = 7)
conf_med[[1]]+ggtitle("med")
conf_med[[2]]+ggtitle("med")
...
pred_med_group = Get_predictive_group(med,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1), n_draws = 2000, model = "med")
pred_med_group_new_sub = Get_predictive_new_subject(med,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1), n_draws = 2000, model = "med")
...
group_predictive(pred_pure_group,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1))+
  labs(
    ...
    subtitle = "Expected response for the population; Where is the mean?",
...
group_predictive(pred_med_group,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1))+
  labs(
    title = expression(paste("(MED) Population mean predictions: E[Y | X, ", mu[theta], "], Uncertainty = Var(",mu[theta],")")),
    subtitle = "Expected response for the population; Where is the mean?",
    caption = "Model predictions for the expected population mean response. 
    Lines ([95 , 90 , 80]% CI) show the posterior mean of the expected value,
    reflecting uncertainty in the group-level parameters."
...
group_predictive(pred_full_group_new_sub,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1))+
  labs(
    ...
    subtitle = "Expected response for hypothetical subjects from the population; Where are individuals",
...
group_predictive(pred_med_group_new_sub,df %>% filter(cohort == "vmp1"& !subject %in% removers_vmp1))+
  labs(
    title = expression(paste(
      "(MED) New subject predictions: ", 
      "E[Y | X, ", theta[s], "] where ", 
      theta[s], " ~ MVN(", mu[theta], ", ", Sigma, ")"
    )),
    subtitle = "Expected response for hypothetical subjects from the population; Where are individuals",
