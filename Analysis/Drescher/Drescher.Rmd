...
bind_rows(
  ...
  df %>%
    mutate(contrast_bin = cut(contrast, 10)) %>%
    group_by(contrast_bin, correct,subject_id) %>%   # <-- group by correctness only here
    summarize(name = "responseConf",
              mean = mean(responseConf),
              se = sd(responseConf) / sqrt(n()))
) %>% 
...
lapply(draws, function(d) {
  ...
  x = seq(min(data$contrast), max(data$contrast), by = 0.1)
...
predictions %>% 
  ...
  geom_point(data = df %>% mutate(draw = NA), aes(x = contrast, y = rt, group = draw))+
  facet_wrap(~subject_id, scales = "free") +
  theme_classic(base_size = 16)
