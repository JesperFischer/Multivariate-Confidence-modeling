...
Get_predictive_ACC_hier = function(fit, df, n_draws = 50) {
  ...
  pred_list <- future_lapply(subjects, function(s) {
    lapply(draws, function(d) {
      ...
      R = matrix(c(1, params$rho_p_rt,
                   params$rho_p_rt, 1), nrow = 2, byrow = TRUE)

      # Sample multivariate normals
      z_samples = MASS::mvrnorm(n = n_trials, mu = rep(0, 2), Sigma = R)
...
Get_predictive_group = function(fit, df, n_draws = 50) {
  ...
  parameters = c("alpha", "beta", "lapse",
                 "rt_int", "rt_slope", "rt_prec","rt_stim")
  ...
  constants = as_draws_df(fit$draws(c("rt_ndt",
                                      "rho_p_rt"))) %>%
    select(-contains(".")) %>%
    mutate(draw = 1:n()) %>%
    pivot_longer(-draw) %>%
  ...
  pred_list <- future_lapply(draws, function(d) {
    ...
    R = matrix(c(1, constants$rho_p_rt,
                 constants$rho_p_rt, 1), nrow = 2, byrow = TRUE)
    ...
    # Calculate expected RT mean
    rt_mu_expected = exp(rt_mu + exp(params$rt_prec)^2 / 2) + constants$rt_ndt + 0.07
...
Plot_group_predictive_psycho = function(predictions, df) {
  ...
  plot_mean = predictionsq_mean %>%
    ...
    geom_pointrange(data = dataq, aes(x = X, y = mean, ymin = q5, ymax = q95),
  ...
  ggsave(here::here("FULL_RT.tiff"),combined_plot ,dpi = 300,width = 20, height = 16, units = "cm")
