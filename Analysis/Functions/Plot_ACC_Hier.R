...
Get_predictive_ACC_hier = function(fit, df, n_draws = 50) {
  ...
  pred_list <- future_lapply(subjects, function(s) {
    lapply(draws, function(d) {
      ...
      # Generate predictions with copula correlations
      x = data$X
      n_trials = length(x)
      ...
      # 1. Decision (accuracy) - sample whether response was CORRECT
      # In the model, binom_y represents whether they got it correct
      # so we sample ACC directly from the copula
      ...
      conf_mu_actual = get_conf(ACC_pred, theta_conf, x, params$alpha)
      ...
      conf_mu_logit = qlogis(pmax(pmin(conf_mu_actual, 0.999), 0.001)) + params$meta_bias
      conf_mu_biased = brms::inv_logit_scaled(conf_mu_logit)
      ...
      predictions = data.frame(
        X = x,
        ACC = ACC_pred,  # 1 if correct, 0 if incorrect
        prob_cor = prob_cor,
        RT_pred = RT_pred,
...
Plot_psychometric_ACC_hier = function(predictions, df, bin = 7) {
...
Plot_RT_ACC_hier = function(predictions, df, bin = 7) {
...
Plot_Conf_ACC_hier = function(predictions, df, bin = 7) {
  ...
  # Predicted data summary
  pred_summary <- predictions %>%
    group_by(X, subject, ACC, draw) %>%
    summarize(Conf_mean = mean(conf_mu_biased), .groups = "drop") %>%
    rename(Correct = ACC)
...
Get_predictive_group = function(fit, df, n_draws = 50) {
  ...
  pred_list <- future_lapply(draws, function(d) {
    ...
    theta_conf = psycho_ACC(x, params$alpha, exp(params$beta + params$meta_un),
                           brms::inv_logit_scaled(params$lapse) / 2)
    ...
    # Apply meta_bias in logit space
    conf_mu_logit = qlogis(pmax(pmin(conf_mu_raw, 0.999), 0.001)) + params$meta_bias
    conf_mu_biased = brms::inv_logit_scaled(conf_mu_logit)
