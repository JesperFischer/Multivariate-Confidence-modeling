    ...
    for (n in 1:N) {
      ...
      if (is_upper == 0) {  // Lower bound
        if (conf[n] == 1) {
          ...
          u_bounds[n, 1] = inv_logit(cutpoints[S_id[n], conf[n] - 1] - logit(theta) + meta_bias[S_id[n]]);
        }
      } else {  // Upper bound
        if (conf[n] == K) {
          u_bounds[n, 1] = 1.0;
        } else {
          u_bounds[n, 1] = inv_logit(cutpoints[S_id[n], conf[n]] - logit(theta) + meta_bias[S_id[n]]);
        }
      }
