functions {
  ...
  matrix uvar_bounds(array[] int binom_y, vector gm, vector X, int is_upper) {
    int N = size(binom_y);
    ...
    for (n in 1:N) {
      real theta = psycho(X[n], alpha , beta , lapse );
      if (is_upper == 0) {
...
model {
  ...
  for (n in 1:N) {
    ...
    real entropy_t = entropy(psycho(X[n], alpha, exp(beta), lapse));

    real conf_mu = get_conf(ACC[n], entropy_conf(psycho(X[n], alpha, exp(beta), lapse),conf_ent));

    real rt_pred =  rt_int + rt_slope * entropy_t + rt_stim * X[n];

    u_mix[n, 1] = u[n,1];

    u_mix[n, 2] = lognormal_cdf(RT[n] - rt_ndt |rt_pred, rt_prec);

    u_mix[n, 3] = ord_beta_reg_cdf(Conf[n] | (logit(conf_mu)), conf_prec, c0, c11);

    target += lognormal_lpdf(RT[n] - rt_ndt | rt_pred, rt_prec);

    target += ord_beta_reg_lpdf(Conf[n] | (logit(conf_mu)), conf_prec, c0, c11);
...
generated quantities {
 ...
 matrix[N, 3] u_mixx;
  for (n in 1:N) {
    ...
    u_mixx[n, 2] = lognormal_cdf(RT[n] - rt_ndt | rt_int + rt_slope * entropy(psycho(X[n], alpha, exp(beta), lapse)) + rt_stim * X[n], rt_prec);

    u_mixx[n, 3] = ord_beta_reg_cdf(Conf[n] | logit(get_conf(ACC[n], entropy_conf(psycho(X[n], alpha, exp(beta), lapse),conf_ent))), conf_prec, c0, c11);
  }
  ...
  for(n in 1:N){
    log_lik_bin[n] = binomial_lpmf(binom_y[n] | 1, psycho(X[n], alpha, exp(beta), lapse));
    log_lik_rt[n] = lognormal_lpdf(RT[n] - rt_ndt | rt_int + rt_slope * entropy(psycho(X[n], alpha, exp(beta), lapse))+ rt_stim * X[n], rt_prec);
    log_lik_conf[n] = ord_beta_reg_lpdf(Conf[n] | logit(get_conf(ACC[n], entropy_conf(psycho(X[n], alpha, exp(beta), lapse),conf_ent))), conf_prec, c0, c11);
