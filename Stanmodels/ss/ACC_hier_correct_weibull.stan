functions {
  ...
  real psycho_ACC(real x, real alpha, real beta, real lapse){
   return (0.5+0.5*(1-2*lapse) * (1-exp(-(x / alpha) ^ beta)));
   }
  real entropy(real p){
  ...
  matrix uvar_bounds(array[] int binom_y, vector gm, vector tau_u,matrix L_u, matrix z_expo, array[] int S_id, vector X,
    ...
    for (n in 1:N) {
      real theta = psycho_ACC(X[n], exp(alpha[S_id[n]]), exp(beta[S_id[n]]), lapse[S_id[n]]);
      if (is_upper == 0) {
  ...
  real get_conf(real ACC, real theta){
  if(ACC == 1){
    return(theta);
  }else if(ACC == 0){
    return(1-theta);
...
transformed parameters{
  ...
  profile("likelihood") {
  for (n in 1:N) {
  theta[n] = psycho_ACC(X[n], exp(alpha[S_id[n]]), exp(beta[S_id[n]]), lapse[S_id[n]]);

  entropy_t[n] = entropy(psycho_ACC(X[n], exp(alpha[S_id[n]]), exp(beta[S_id[n]]), lapse[S_id[n]]));

  theta_conf[n] = psycho_ACC(X[n], exp(alpha[S_id[n]]), exp(beta[S_id[n]] + meta_un[S_id[n]]), lapse[S_id[n]]);

  conf_mu[n] = get_conf(ACC[n],theta_conf[n]);
  }
  }
