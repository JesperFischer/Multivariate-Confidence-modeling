functions {
  ...
  real psycho_ACC(real x, real alpha, real beta, real lapse){
   ...
   real psycho_lapse_free(real x, real alpha, real beta){
    return (inv_logit(beta * (x - alpha)));
   }

  real entropy(real p){
    // Clamp p to avoid log(0) which produces NaN (needed for lapse-free psychometric functions)
    real p_safe = fmax(fmin(p, 0.9999), 0.0001);
    return(-p_safe * log(p_safe) - (1-p_safe) * log(1-p_safe));
  }
...
transformed parameters{
  ...
  entropy_t[n] = entropy(psycho_lapse_free(X[n], (alpha[S_id[n]]), exp(beta[S_id[n]])));

  theta_conf[n] = psycho_lapse_free(X[n], (alpha[S_id[n]]), exp(beta[S_id[n]] + meta_un[S_id[n]]));

  // Clamp conf_mu to prevent logit from being infinite (since theta_conf can be 0 or 1 without lapse)
  conf_mu[n] = fmax(fmin(get_conf(ACC[n],theta_conf[n], X[n], alpha[S_id[n]]), 0.9999), 0.0001);
  }
  }
