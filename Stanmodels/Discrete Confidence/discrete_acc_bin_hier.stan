functions {
  ...
  matrix uvar_bounds_conf(array[] int conf, vector gm, vector tau_u,matrix L_u, matrix z_expo, array[] int S_id, vector X, vector ACC, array[] vector cutpoints,
    ...
    for (n in 1:N) {
      ...
      real conf_mu = get_conf(ACC[n],theta_conf, X[n], alpha[S_id[n]]);
      ...
      if (is_upper == 0) {  // Lower bound
        if (conf[n] == 1) {
          ...
          u_bounds[n, 1] = inv_logit(cutpoints[S_id[n], conf[n] - 1] - (logit(conf_mu) + meta_bias[S_id[n]]));
        }
      } else {  // Upper bound
        if (conf[n] == K) {
          u_bounds[n, 1] = 1.0;
        } else {
          u_bounds[n, 1] = inv_logit(cutpoints[S_id[n], conf[n]] - (logit(conf_mu) + meta_bias[S_id[n]]));
        }
      }
