...
transformed parameters{
  ...
  vector[S] meta_un = param[,13];
  vector[S] rt_un = param[,14];
...
model {
  ...
  gm[8:12] ~ normal(0,5); //global mean of beta
  gm[13:14] ~ normal(0,3); //global mean of beta

  to_vector(z_expo) ~ std_normal();

  tau_u[1] ~ normal(3 , 3);
  tau_u[2:6] ~ normal(0 , 3);
  tau_u[7] ~ normal(0 , 0.1);
  tau_u[8:12] ~ normal(0 , 3);
  tau_u[13:14] ~ normal(0 , 1);
...
generated quantities {
 ...
 matrix[N, 3] u_mixx;
  for (n in 1:N) {
    ...
    u_mixx[n, 2] = lognormal_cdf(RT[n] - rt_ndt[S_id[n]] | rt_int[S_id[n]] + rt_slope[S_id[n]] * entropy(psycho(X[n], alpha[S_id[n]], exp(beta[S_id[n]] + rt_un[S_id[n]]), lapse[S_id[n]])) + rt_stim[S_id[n]] * X[n], rt_prec[S_id[n]]);

    u_mixx[n, 3] = ord_beta_reg_cdf(Conf[n] |
    conf_int[S_id[n]] + conf_ACC[S_id[n]] * ACC[n] + conf_entropy[S_id[n]] * entropy(psycho(X[n], alpha[S_id[n]], exp(beta[S_id[n]] + meta_un[S_id[n]]), lapse[S_id[n]])) + conf_entropy_ACC[S_id[n]] * ACC[n] * entropy(psycho(X[n], alpha[S_id[n]], exp(beta[S_id[n]]+ meta_un[S_id[n]]), lapse[S_id[n]])),
  ...
  for(n in 1:N){
    ...
    log_lik_rt[n] = lognormal_lpdf(RT[n] - rt_ndt[S_id[n]] | rt_int[S_id[n]] + rt_slope[S_id[n]] * entropy(psycho(X[n], alpha[S_id[n]], exp(beta[S_id[n]] + rt_un[S_id[n]]), lapse[S_id[n]]))+ rt_stim[S_id[n]] * X[n], rt_prec[S_id[n]]);
    log_lik_conf[n] = ord_beta_reg_lpdf(Conf[n] | conf_int[S_id[n]] +
    conf_ACC[S_id[n]] * ACC[n] +
    conf_entropy[S_id[n]] * entropy(psycho(X[n], alpha[S_id[n]], exp(beta[S_id[n]]+ meta_un[S_id[n]]), lapse[S_id[n]])) + conf_entropy_ACC[S_id[n]] * ACC[n] * entropy(psycho(X[n], alpha[S_id[n]], exp(beta[S_id[n]]+ meta_un[S_id[n]]), lapse[S_id[n]])),
    conf_prec[S_id[n]], c0[S_id[n]], c11[S_id[n]]);
