...
dfd <- read_csv(here::here("Data","quad","raw_delayed.csv"))
...
```{r, fig.height=7,fig.width=12}
df = df %>% filter(trial_type == "decision" & bias_source == "mullerlyer" & bias_direction == "long") %>% 
    mutate(X = reference_length-target_length) %>% 
    mutate(Decision = ifelse(answer == "short",1,0)) %>% 
    mutate(confidence = ifelse(confidence == "low",0,1)) %>%
    rename(
...
```{r, fig.height=7,fig.width=12, message=F, error=F,warning=F}
lapply(list(1:10), function(chunk) plot_subjects(df %>% filter(!subject %in% removers), chunk, bin = 7))

```

```{r, fig.height=7,fig.width=12}

df1 = df %>% filter(!subject %in% !subject %in% removers) %>% 
  filter(subject < 8) %>% filter(target_length != 400)
