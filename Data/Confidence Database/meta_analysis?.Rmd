...
allfiles = list.files(here::here("Data","Confidence Database","Confidence Database"), full.names = T)

csvs = allfiles[grepl(".csv",allfiles)]
txts = allfiles[grepl(".txt",allfiles)]
...
read.delim(txts[1])
read.csv(csvs[1]) %>% ggplot(aes(x = Orientation ,fill = as.factor(Stimulus)))+geom_histogram(position = "identity", alpha = 0.5)+facet_wrap(Difficulty~Task)
read.csv(csvs[1])%>%
...
```{r, fig.height=10,fig.width=10}
read.delim(txts[2])
read.csv(csvs[2]) %>% ggplot(aes(x = Orientation ,fill = as.factor(Stimulus)))+geom_histogram(position = "identity", alpha = 0.5)+facet_wrap(Difficulty~Task)
read.csv(csvs[2])%>%
  mutate(Difficulty = ifelse(Stimulus == 2, 13 - Difficulty, Difficulty),
         ACC = ifelse(Stimulus == Response,1,0),
         Response = Response-1,
...
read.csv(csvs[2]) %>%
  mutate(Difficulty = ifelse(Stimulus == 2, 13 - Difficulty, Difficulty),
         ...
         Response = Response-1,
         Confidence = Confidence/max(Confidence)) %>% 
  pivot_longer(cols = c(Response,Confidence,RT_dec))%>% group_by(Difficulty,name,Task,ACC) %>% 
  filter(name == "Confidence") %>% 
  summarize(mean = mean(value),
            se = sd(value)/sqrt(n())) %>% 
  ggplot(aes(x = Difficulty, col = as.factor(ACC)))+
...
```{r, fig.height=10,fig.width=10}
read.delim(txts[3])
read.csv(csvs[3]) %>% ggplot(aes(x = Orientation ,fill = as.factor(Stimulus)))+geom_histogram(position = "identity", alpha = 0.5)+facet_wrap(~Difficulty)
...
read.csv(csvs[13]) %>% 
  ...
  filter(name == "Confidence") %>% 
  mutate(Difficulty_bin = cut(Difficulty, breaks = 20, labels = FALSE)) %>% 
  group_by(Difficulty_bin ,Stimulus,name,Condition,ACC) %>% 
  summarize(mean = mean(value,na.rm = T),
            se = sd(value,na.rm = T)/sqrt(n())) %>% 
  ggplot(aes(x = Difficulty_bin , col = as.factor(ACC)))+
  geom_pointrange(aes(y = mean,ymin = mean-2*se, ymax = mean+2*se))+
  facet_wrap(~Condition, scales = "free")+theme_minimal()+
  theme(legend.position = "top",
  text = element_text(size = 16),        # General text size
  axis.title = element_text(size = 18), # Axis titles
  axis.text = element_text(size = 14),  # Axis tick labels
...
```{r, fig.height=10,fig.width=10}
read.delim(txts[14])
read.csv(csvs[14])%>% filter(!Subj_idx %in% 179:201) %>% 
  group_by(Task) %>% 
         mutate(ACC = ifelse(Stimulus == Response,1,0),
                Difficulty  = ifelse(Stimulus == 1, ContrastOrTilt , -ContrastOrTilt),
                Confidence = Confidence) %>% 
  ungroup() %>% 
  pivot_longer(cols = c(Response,Confidence,RT_dec))%>% 
  mutate(Difficulty_bin = cut(Difficulty, breaks = 9, labels = FALSE)) %>%
  group_by(Difficulty_bin ,Stimulus,name,Task) %>% 
  summarize(mean = mean(value,na.rm = T),
            se = sd(value,na.rm = T)/sqrt(n())) %>% 
  ggplot(aes(x = Difficulty_bin , col = as.factor(name)))+
  geom_pointrange(aes(y = mean,ymin = mean-2*se, ymax = mean+2*se))+
  facet_wrap(Task~name, scales = "free")+theme_minimal()+
  theme(legend.position = "top",
  text = element_text(size = 16),        # General text size
  axis.title = element_text(size = 18), # Axis titles
  axis.text = element_text(size = 14),  # Axis tick labels
...
read.csv(csvs[14])%>% filter(!Subj_idx %in% 179:201) %>% 
  ...
  mutate(Difficulty_bin = cut(Difficulty, breaks = 5, labels = FALSE)) %>% 
    group_by(Difficulty_bin ,Stimulus,name,Task,ACC) %>% 
  summarize(mean = mean(value,na.rm = T),
            se = sd(value,na.rm = T)/sqrt(n())) %>% 
  ggplot(aes(x = Difficulty_bin , col = as.factor(ACC)))+
  geom_pointrange(aes(y = mean,ymin = mean-2*se, ymax = mean+2*se))+
...
read.csv(csvs[20])%>% 
  ...
  mutate(Difficulty_bin = cut(Difficulty, breaks = 10, labels = FALSE)) %>%
    filter(name == "Confidence") %>% 
  group_by(Difficulty_bin ,Stimulus,name,ACC) %>% 
  summarize(mean = mean(value,na.rm = T),
            se = sd(value,na.rm = T)/sqrt(n())) %>% 
  ggplot(aes(x = Difficulty_bin , col = as.factor(ACC)))+
  geom_pointrange(aes(y = mean,ymin = mean-2*se, ymax = mean+2*se))+
  facet_wrap(~Stimulus, scales = "free")+theme_minimal()+
  theme(legend.position = "top",
  text = element_text(size = 16),        # General text size
  axis.title = element_text(size = 18), # Axis titles
  axis.text = element_text(size = 14),  # Axis tick labels
...
df %>% 
  ...
  pivot_longer(cols = c(Response,Confidence,RT_dec))%>% 
  filter(name == "Confidence") %>% 
  mutate(Difficulty_bin = cut(Difficulty, breaks = 6)) %>% 
  group_by(Difficulty_bin ,Stimulus,Subj_idx,Condition, ACC) %>% 
  summarize(mean = mean(value,na.rm = T),
            se = sd(value,na.rm = T)/sqrt(n())) %>% 
  ggplot(aes(x = Difficulty_bin , col = as.factor(ACC)))+
  geom_pointrange(aes(y = mean,ymin = mean-2*se, ymax = mean+2*se))+
  facet_wrap(Subj_idx~Condition, scales = "free")+theme_minimal()+
  theme(legend.position = "top"
)
